...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		// #Specialmove Promoted
				...
				if(movedPiece instanceof Pawn) {
					...
					promoted = replacePromotedPieces("Q");
					}
				}
	...
	public ChessPiece replacePromotedPieces(String type) {
		if(promoted == null) {
	...
	private Piece makeMove(Position source, Position target) {
		...
		if (p instanceof King && target.getColumn()== source.getColumn() + 2) {
			Position sorceT = new Position(source.getRow(), source.getColumn() + 3);
		...
		if (p instanceof King && target.getColumn()== source.getColumn() - 2) {
			Position sorceT = new Position(source.getRow(), source.getColumn() - 4);
			Position targetT = new Position(source.getRow(), source.getColumn() - 1);
			ChessPiece rook = (ChessPiece)board.removePiece(sorceT);
			board.placePiece(rook, targetT);
			rook.increaseMoveCount();
	...
	private void validateSourcePosition(Position position) {
		...
		if (!board.piece(position).isThereAnyPossibleMove()) {
			throw new ChessException("There is not possibles moves for the chosen piece");
		}
	}
